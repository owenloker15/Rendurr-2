...
namespace Rendurr
{
	...
	struct FramebufferSpecification
	{
		uint32_t width;
		uint32_t height;
		std::unordered_map<std::string, ColorAttachmentFormat> m_colorAttachments;  // id (not used but for access)
	};
	...
	private:
		void addColorAttachment(const std::string& colorAttachmentKey, const ColorAttachmentFormat& format);
	private:
